...
<section class="section">
    <div class="section-body">
        ...
        <div class="card mt-0">
            <form action="{{ url('saveeditsmt/'.$smt->id_smt) }}"  method="POST" class="">
                @csrf
                @method('PUT')
              <div class="card-header">
                <h4 class="section-title" >Form Edit Data Semester </h4>
              </div>
              <div class="card-body">
                <div class="form-group row">
                  <label class="col-sm-3 col-form-label">Nama Semester</label>
                  <div class="col-sm-4"> <input type="text" class="form-control" name="nama_smt" value="{{ $smt->nama_smt }}" required>
                  </div>
                  <p class="text-danger">*isi semester berupa angka, contoh : 1</p>
                </div>
                <div class="form-group row">
                  <label class="col-sm-3 col-form-label">Tahun</label>
                  <div class="col-sm-4"> <input type="text" class="form-control" name="tahun" value="{{ $smt->tahun }}" required>
                  </div>
                ...
                <div class="form-group row">
                    ...
                    <select class="form-control" name="ket_smt">
                        <option value="ganjil" {{ $smt->ket_smt == "ganjil" ? 'selected' : '' }}>ganjil</option>
                        <option value="genap" {{ $smt->ket_smt == "genap" ? 'selected' : '' }}>genap</option>
                    </select>
                    </div>
                </div>
              </div>
