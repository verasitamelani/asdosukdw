...
<section class="section">
    <div class="section-body">
        ...
        <div class="card mt-0">
            <form action="{{ url('dtdetaileditup/'.$detail->id_detail) }}"  method="POST" class="">
                @csrf
                @method('PUT')
              <div class="card-header">
                <h4 class="section-title" >Form Edit Data Detail Kelas </h4>
              </div>
              <div class="card-body">
                <div class="form-group row">
                    ...
                    @foreach($matkul as $mk)
                        <option value="{{ $mk->id_kelas }}" @if($mk->id_kelas== $detail->id_kelas) selected @endif>{{ $mk->nama_mk }} - {{ $mk->grup }}</option>
                    @endforeach
                    </select>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    @foreach($users as $user)
                      <option value="{{ $user->id }}" @if($user->id== $detail->id) selected @endif>{{ $user->nama }} - {{ $user->nama_prodi }}</option>
                    @endforeach
                    </select>
                    </div>
                </div>
